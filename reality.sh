...
function config_xray() {
  ...
  local xs_public_key=$(echo "$xray_x25519" | grep '^Password (PublicKey):' | awk '{print $2}')
...
function menu() {
  ...
  if [ -d /usr/local/etc/xray-script ] && ([ ${idx} -gt 102 ] || [ ${idx} -lt 111 ]); then
    wget -qO ${xray_config_manage} https://raw.githubusercontent.com/FlaminG0/Xray-script/main/tool/xray_config_manage.sh
  ...
  1)
    if [[ ! -d /usr/local/etc/xray-script ]]; then
      ...
      wget -O /usr/local/etc/xray-script/config.json https://raw.githubusercontent.com/FlaminG0/Xray-script/main/config/config.json
      wget -O ${xray_config_manage} https://raw.githubusercontent.com/FlaminG0/Xray-script/main/tool/xray_config_manage.sh
      chmod a+x ${xray_config_manage}
  ...
  101)
    ...
    ;;
  102)
    [[ -f /usr/local/etc/xray-script/traffic.sh ]] || wget -O /usr/local/etc/xray-script/traffic.sh https://raw.githubusercontent.com/FlaminG0/Xray-script/main/tool/traffic.sh
  ...
  204)
    ...
    if [[ ${is_opt} =~ ^[Yy]$ ]]; then
      ...
      wget -O /etc/sysctl.conf https://raw.githubusercontent.com/FlaminG0/Xray-script/main/config/sysctl.conf
      sysctl -p
    fi
    ;;
  0)
